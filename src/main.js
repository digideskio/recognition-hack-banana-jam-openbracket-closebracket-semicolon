...
twitter.onTweet((tweet, sendIfEmpty) => {
	if (tweet && tweet.entites && tweet.entities.media && tweet.entities.media.length > 0){
		...
		cognition.recognize(url).then((response) => {
			...
			if (message){
				...
				if (mood == 'noface'){
					console.log('not tweeting response because no face was detected');
				}
			}
		});
	}
